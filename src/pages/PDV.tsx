...
import {
  Search, Plus, Minus, Trash2, CreditCard, Banknote, QrCode, Smartphone, X, RotateCcw,
} from "lucide-react";
...
export default function PDV() {
  ...
  const finalizeSale = (paymentMethod: string) => {
    ...
    toast({
      ...
      description: `${totalItems} itens — ${formatBRL(subtotal)} via ${paymentMethod}. Estoque atualizado.`,
    });
    clearCart();
  };
  ...
  const handleCancelSale = () => {
    if (lastSaleId) {
      ...
      toast({ title: "Venda cancelada", description: "Estoque restaurado com sucesso." });
      setLastSaleId(null);
    }
    ...
    <div className="flex h-screen bg-pos-bg">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="h-14 border-b border-border bg-card flex items-center px-4 gap-3">
          ...
          {lastSaleId && (
            <Button variant="outline" size="sm" onClick={() => setCancelDialogOpen(true)} className="ml-2 text-destructive border-destructive/30 hover:bg-destructive/10">
              <RotateCcw className="h-3.5 w-3.5 mr-1" /> Cancelar última
            </Button>
          )}
        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2">
            {filteredProducts.map((product) => (
              <button
                ...
                className="rounded-md border border-border bg-pos-card p-3 text-left transition-all duration-100 hover:border-primary active:scale-[0.98] flex flex-col disabled:opacity-40 disabled:cursor-not-allowed"
              >
            ...
            {cart.map((item) => (
              ...
              >
                <div className="flex justify-between items-start">
                  ...
                  <button onClick={() => removeItem(item.product.id)} className="text-muted-foreground hover:text-destructive p-1 -mr-1">
                    <Trash2 className="h-3.5 w-3.5" strokeWidth={1.5} />
                  </button>
                </div>
                <div className="flex items-center justify-between mt-2">
                  <div className="flex items-center gap-1">
                    <button onClick={() => updateQty(item.product.id, -1)} className="h-8 w-8 rounded-md border border-border flex items-center justify-center hover:bg-secondary transition-colors">
                      <Minus className="h-3 w-3" />
                    </button>
                    <span className="w-8 text-center text-sm tabular-nums font-medium text-foreground">{item.quantity}</span>
                    <button onClick={() => updateQty(item.product.id, 1)} className="h-8 w-8 rounded-md border border-border flex items-center justify-center hover:bg-secondary transition-colors">
            ...
            <div className="space-y-2 animate-fade-in">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <button
                    key={method.label}
                    onClick={() => finalizeSale(method.label)}
              ...
              <button onClick={() => setShowPayment(false)} className="w-full text-xs text-muted-foreground hover:text-foreground py-1">
                Cancelar
              </button>
            </div>
          )}
      ...
      <Dialog open={cancelDialogOpen} onOpenChange={setCancelDialogOpen}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              O estoque será restaurado automaticamente. Deseja continuar?
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
