...
import {
  Banknote, QrCode, CreditCard, Smartphone, Plus, X, Calculator,
} from "lucide-react";
...
export default function PaymentPanel({ total, onFinalize, onCancel }: PaymentPanelProps) {
    ...
    <div className="space-y-3 animate-fade-in">
      ...
      {payments.length === 0 && (
        <>
          ...
          <button
            onClick={() => setSelectedMethod("_mixed")}
              ...
              {selectedMethod && selectedMethod !== "_mixed" && (
                <div className="flex gap-2">
                  <Input
                    type="number"
                    inputMode="decimal"
                    placeholder={`Valor (max ${formatBRL(remaining)})`}
                    value={partialAmount}
                    onChange={(e) => setPartialAmount(e.target.value)}
                    className="h-10 text-sm flex-1"
                    autoFocus
                  />
                  <Button size="sm" onClick={addPartialPayment} className="h-10 touch-manipulation">
                    <Plus className="h-4 w-4" />
                  </Button>
                  <Button size="sm" onClick={() => { addFullPayment(selectedMethod); setSelectedMethod(null); }} variant="outline" className="h-10 touch-manipulation text-xs">
                    Tudo
                  </Button>
                </div>
              )}
            ...
            <div className="border border-border rounded-lg p-3 space-y-2">
              ...
              {cashReceivedNum > 0 && (
                <div className="flex justify-between items-baseline">
                  ...
                  <span className={`text-lg font-bold tabular-nums ${change > 0 ? "text-success" : "text-foreground"}`}>
                    {formatBRL(change)}
                  </span>
                </div>
              )}
      ...
      <div className="flex gap-2">
        <button onClick={onCancel} className="flex-1 text-xs text-muted-foreground hover:text-foreground py-2 touch-manipulation">
          Cancelar (Esc)
        </button>
        {totalPaid >= total && (
          <Button onClick={handleFinalize} disabled={!canFinalize} className="flex-1 h-10 touch-manipulation">
            Confirmar
          </Button>
        )}
      </div>
